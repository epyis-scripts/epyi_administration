		...
		if CurrentMenu() then
			...
			if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
				...
				local ProgressText = (
					...
					or (type(Items[ProgressStart]) == "table") and tostring(Items[ProgressStart].Name)
				...
				if Enabled == true or Enabled == nil then
					if Selected then
						RenderText(
							ProgressText,
							CurrentMenu.X + SettingsButton.RightText.X + CurrentMenu.WidthOffset,
						...
						RenderRectangle(
							...
							CurrentMenu.Y
								...
								+ CurrentMenu.SubtitleHeight
								+ RageUI.ItemOffset,
							SettingsProgress.Background.Width + CurrentMenu.WidthOffset,
				...
				if
					...
					and (
						...
						or ((Hovered and CurrentMenu.Controls.Click.Active) and not ProgressHovered)
					)
				then
					local Audio = RageUI.Settings.Audio
				...
				elseif Selected and (Hovered and CurrentMenu.Controls.Click.Active and ProgressHovered) then
					---@type number
					local Progress = (math.round(GetControlNormal(2, 239) * 1920) - CurrentMenu.SafeZoneSize.X)
